Shader "NatureManufacture Shaders/Ice Lake/Ice Shader Specular Lake"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float simplePerlin3D342 = snoise( ( ase_worldPos / _IceNoiseTiling ) );
			float4 lerpResult249 = lerp( ( saturate( 	max( blendOpSrc236, blendOpDest236 ) )) , temp_output_230_0 , simplePerlin3D342);
			...
			float eyeDepth346 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture,UNITY_PROJ_COORD( ase_screenPos )));
			float temp_output_94_0 = saturate( pow( ( abs( ( eyeDepth346 - ase_screenPos.w ) ) * _WaterDepth ) , _WaterFalloff ) );
			...
			half4 screenColor305 = tex2D( GrabScreen1, ( (ase_screenPosNorm).xy + (( tex2DNode17 * _ShadowRefractionPower )).xy ) );
			...
			float clampResult357 = clamp( ( clampResult299 * _ShadowSpecularOcclusionPower ) , 0.0 , 1.0 );
			float lerpResult354 = lerp( _SpecularPower , _ShadowSpecularPower , clampResult357);
			#ifdef _SHADOWSPECULAROCCLUSION_ON
				float staticSwitch356 = lerpResult354;
			#else
				float staticSwitch356 = _SpecularPower;
			#endif
			float3 temp_cast_3 = (staticSwitch356).xxx;
